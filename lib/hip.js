  ...
  if(objects.length>0){
    ...
    head.prepend('<style>' +
      ...
      '.hip-item{padding: '+itemGaps+';}' +
        '.hip-item-hidden{display: none !important;}' +
      '</style>');
    ...
    $.each( objects, function( key,curObj ) {
      ...
      if(filter){
        ...
        $(curObj).before('<div class="hip-filter">\n' +
...
$(".hip-search-input").on('focus', function () {
  $(this).parent('label').addClass('active');
});

$(".hip-search-input").on('blur', function () {
...
$(document).on('keyup','.hip-search-input', function () {
  ...
  var filteredLen = Math.ceil(getFilteredCount(items,$(this).val())/itemsPerPage);
  ...
  items.filter(function() {
    ...
    if(typeof tags !== "undefined")
    {
      ...
      tags = tags.toLowerCase().split(" ");
       ...
       else if ($(this).html().trim().toLowerCase().indexOf(value) > -1){
         init_filter++;
         $(this).removeClass("hip-item-out");
       }
       else {
